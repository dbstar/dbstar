...
#define MAC_FILENAME "cytcmac.csv"
#define MAC_NUM_FILENAME "cytcmacnum.csv"
#define MAC_NUM_LEN 12  //8+4
...
//printf("liukevin %x %x  %x\n",*p,'0','9');
    ...
    //add end
    while(my_isdigit(*p)) res = res*10 + (*p++ - '0');

    return neg_flag ?0 -res : res;
}
//liukevin add

int tc_aml_writemac(void)
{
        int     i = 0;
        char    str[128];
...
//printf("liukevin find a MAC file in SD CARD\n");
                            ...
                            if (!run_command (str, 0))
                            {
                                ...
                                sprintf(str, "fatload usb 0:%d 82000000 %s", (i + 1), MAC_NUM_FILENAME);
                                run_command (str, 0);
                                mac = (unsigned char *)0x82000000;
                                sprintf(str, "%c%c%c%c%c%c%c%c",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5],mac[6],mac[7]);
                                mac_num = my_atoi(str);
                                mac = (unsigned char *)(0x82000000+MAC_NUM_LEN*(mac_num+1));
                                sprintf(str, "%c%c%c%c%c%c%c%c",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5],mac[6],mac[7]);
                                mac_num = my_atoi(str);

printf("----------------------mac_num = %d\n",mac_num);
                                sprintf(str, "fatload usb 0:%d 82200000 %s", (i + 1), MAC_FILENAME);
                                run_command (str, 0);
                                addr = (unsigned char *)(0x82200000+14*mac_num);
...
#define CHANGE_MAC_STB_NUM "change_MAC_stb_NUM"
...
//printf("usb start ......\n");
	...
	else
	{
		for(i = 0; i < SCAN_USB_PARTITION; i++)
		{
			sprintf(str, "fatexist usb 0:%d %s", (i + 1), UPDATE_MAKR_FILE);
			if(!run_command (str, 0))
			{
				sprintf(str, "fatload usb 0:%d ${loadaddr} %s", (i + 1), "recovery.img");
				if(!run_command (str, 0))
                                {
				    run_command ("bootm", 0);
                                }
                                else
                                {
                                    ...
                                    run_command ("nand read recovery ${loadaddr} 0 600000", 0);
                                    run_command ("bootm", 0);
                                }
			}
                        sprintf(str, "fatexist usb 0:%d %s", (i + 1), CHANGE_MAC_STB_NUM);
                        if(!run_command (str, 0))
                        {
                                //power_up();
                                //logo_display();
                                ...
                                sprintf(str, "fatload usb 0:%d ${loadaddr} %s", (i + 1), CHANGE_MAC_STB_NUM);
                                run_command (str, 0);
                                run_command ("bootm", 0);
                        }
		}
