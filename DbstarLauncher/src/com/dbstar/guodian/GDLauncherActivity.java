...
public class GDLauncherActivity extends GDBaseActivity implements
	...
	AnimationSet mShowHighlightAnimation, 
			mPopupMenuFocusedAnimation,
	...
	private boolean onItemSelected() {
		...
		Log.d(TAG, "column id=" + menuItem.ColumnId() + " column type=" + menuItem.Type());
	...
	public void updateData(int type, int columnLevel, int index, Object data) {
		...
		if (type == GDDataProviderService.REQUESTTYPE_GETCOLUMNS) {
			...
			Log.d(TAG, "updateData columnLevel " + columnLevel + " columns " + columns);
...
//			}
	...
	void initializeMenu() {
		...
		subColumns[USERCENTER_LEVEL2_MYORDER].Name = getResources()
				.getString(R.string.menulevel2_item_myorder);
