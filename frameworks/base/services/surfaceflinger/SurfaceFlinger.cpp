...
static int videoHoleChanged(void);
...
void SurfaceFlinger::init()
{
	...
	defaultOrientation = atoi(value);

    property_get("ro.flashplayer.surfacehack", value, "0");
    mAdobeFlashSurfaceHack = atoi(value);

    if (mDebugDDMS) {
        DdmConnection::start(getServiceName());
    }
	mCurrentState.orientation=defaultOrientation;
...
void SurfaceFlinger::binderDied(const wp<IBinder>& who)
{
	...
	setOrientation(0, defaultOrientation, 0);
...
status_t SurfaceFlinger::readyToRun()
{
	...
	windowWidth = atoi(valuep)>0? atoi(valuep): plane.getWidth(); 
	property_get("const.window.h", valuep, "0") ;    
	windowHeight = atoi(valuep)>0? atoi(valuep): plane.getHeight(); 
	
    // initialize the shared control block
    mServerCblk->connected |= 1<<dpy;
    display_cblk_t* dcblk = mServerCblk->displays + dpy;
    memset(dcblk, 0, sizeof(display_cblk_t));
    dcblk->w            = windowWidth;//plane.getWidth();
    dcblk->h            = windowHeight;//plane.getHeight();
    dcblk->format       = f;
    dcblk->orientation  = defaultOrientation;//eOrientationDefault;
...
bool SurfaceFlinger::threadLoop()
{
	...
	if(need_unblank_fb0){
		need_unblank_fb0--;
		if(need_unblank_fb0==0){
			//amsysfs_set_sysfs_str(FB0_BLANK_PATH,"0");
			//amsysfs_set_sysfs_str(FB1_BLANK_PATH,"0");
		}
		if(need_unblank_fb0){
			const DisplayHardware& hw(graphicPlane(0).displayHardware());
			mDirtyRegion.set(hw.bounds());
    ...
    const LayerVector& currentLayers, Region& dirtyRegion, Region& opaqueRegion)
{
    ...
    #define BROWSER_ADDRBAR_HEIGHT 56
    bool seeAndroidBrowser = false;
    bool seeStatusBar = false;
    Rect browserBounds(0,0,0,0);
    Rect statusBarBounds(0,0,0,0);
    Rect browserClipBounds(0,0,0,0);

    if (mAdobeFlashSurfaceHack) {
        display_cblk_t* dcblk = mServerCblk->displays;
        int32_t clipHeight = BROWSER_ADDRBAR_HEIGHT / dcblk->density;

        size_t layerNum = currentLayers.size();
        while (layerNum--) {
            const sp<LayerBase>& layer = currentLayers[layerNum];
            if (layer->getName() == "com.android.browser/com.android.browser.BrowserActivity") {
                seeAndroidBrowser = true;
                browserBounds.set(layer->visibleBounds());
                break;

            } else if (layer->getName() == "StatusBar") {
                seeStatusBar = true;
                statusBarBounds.set(layer->visibleBounds());
            }
        }

        if (seeAndroidBrowser && seeStatusBar) {
            Point p1(statusBarBounds.leftTop());
            Point p2(statusBarBounds.rightBottom());
            Point p3(browserBounds.leftTop());
            Point p4(browserBounds.rightBottom());

            if (statusBarBounds.width() > statusBarBounds.height()) {
                if (p1.y == 0) {
                    browserClipBounds.set(Rect(p3.x, p4.y - clipHeight, p4.x, p4.y));
                } else {
                    browserClipBounds.set(Rect(p3.x, p3.y, p4.x, p3.y + clipHeight));
                }
            } else {
                if (p1.x == 0) {
                    browserClipBounds.set(Rect(p4.x - clipHeight, p3.y, p4.x, p4.y));
                } else {
                    browserClipBounds.set(Rect(p3.x, p3.y, p3.x + clipHeight, p4.y));
                }
            }
        }
    }
    ...
    while (i--) {
        ...
        if (LIKELY(!(s.flags & ISurfaceComposer::eLayerHidden) && s.alpha)) {
            ...
            if (mAdobeFlashSurfaceHack && seeAndroidBrowser && seeStatusBar &&
                (layer->getName() == "SurfaceView")) {
                if (browserBounds != bounds) {
                    visibleRegion.subtractSelf(Region(browserClipBounds));
                }
            }
...
int amsysfs_set_sysfs_str(const char *path, const char *val)
{
    ...
    if (fd >= 0) {
		...
		LOGI("amsysfs_set_sysfs_str %s= %s\n", path,val);
        close(fd);
        return 0;
    } else {
    }
...
void SurfaceFlinger::composeSurfaces(const Region& dirty)
{
	...
	if (ret < 0) {
    	...
    	if(sscanf(buf, "%i", &request)!=1)
    	{
    		request=0;
    	}else if(request ==Last2XScaleValue)
    	{
    		request =0;
    	}
	}
//	LOGW("composeSurfaces::Last2XScaleValue %i  request %i",
//                    Last2XScaleValue,request);
	if((request == 1)||((Last2XScaleValue==1)&&(request==0)))
	{

		
		//draw to texturebuffer,deal with it,and then draw to framebuffer,
		preProcessWithFrameBuffer( dirty);
		if(request == 1)
		{
			//echo 0 to REQUEST_SCALE_PATH to make next time it's value is 0;
			amsysfs_set_sysfs_str(REQUEST_SCALE_PATH,"0");
			Last2XScaleValue=request;
			//write to scale ,scale_axis;
			amsysfs_set_sysfs_str(SCALE_AXIS_PATH,"0 0 639 767");
	...
	else
	{
		...
		property_get("hw.videohole.layer", property_tmp, "0") ;
		int layer_number=atoi(property_tmp);
	    for (size_t i=0 ; i<count ; i++) {
...
#define ENFORCE_PROGRESSIVE_PATH       "/sys/class/graphics/fb0/enforce_progressive"
#define SYSCMD_BUFSIZE   32
static int Last2XScaleValue=0;
static int request_width=0;
static int request_height=0;
static char lastcmdbuf[SYSCMD_BUFSIZE] ;
static char lastvhbuf[SYSCMD_BUFSIZE] ;
static int request2XScaleChanged(void)
{
    int ret;
    char buf[SYSCMD_BUFSIZE];
...
static int videoHoleChanged(void)
{
    ...
    ret = amsysfs_get_sysfs_str(REQUEST_VIDEO_HOLE, buf, SYSCMD_BUFSIZE);
    if (ret < 0 || memcmp(buf,lastvhbuf,SYSCMD_BUFSIZE)==0) {
        return 0;
    }
...
//	debugShowFPS();
	...
	int request=0;
    int ret;
    int holex=0,holey=0,holew=0,holeh=0,holelayer=0,iptv=0;

	char property_layer[32];
	property_get("hw.videohole.layer", property_layer, "0") ;
	
	ret = amsysfs_get_sysfs_str(REQUEST_VIDEO_HOLE, videoholebuf, SYSCMD_BUFSIZE);
	if (ret < 0) {
		holelayer=0;
	}
	else{
		memcpy(lastvhbuf,videoholebuf,SYSCMD_BUFSIZE);
		sscanf(videoholebuf, "%i %i %i %i %i %i", &holex,&holey,&holew,&holeh,&holelayer,&iptv);
		
		if(holelayer<=0)holelayer=atoi(property_layer);
	}
    ...
    {
    	memcpy(lastcmdbuf,buf,SYSCMD_BUFSIZE);
    	if(sscanf(buf, "%i", &request)==1)
		{
			if(request>0)
			{
				sscanf(buf, "%i %i %i", &request,&request_width,&request_height);
			}
		}else
		{
			request=0;
...
//	LOGW("composeSurfaces::Last2XScaleValue %i  request %i %i %i",
//                    Last2XScaleValue,request,request_width,request_height);
	if(request == 1)
	{
		mNeed2XScale =true;
		//echo 0 to REQUEST_SCALE_PATH to make next time it's value is 0;
		...
		Last2XScaleValue=request;
		//write to scale ,scale_axis;
	}else if(request == 8)//change to 1080P
	{
		mNeed2XScale =false;
		//amsysfs_set_sysfs_str(REQUEST_SCALE_PATH,"0");
		Last2XScaleValue=request;
	...
	else if(request == 16)//change to 480p
	{
		mNeed2XScale =false;
		//amsysfs_set_sysfs_str(REQUEST_SCALE_PATH,"0");
		Last2XScaleValue=request;		
	...
	const DisplayHardware& hw(graphicPlane(0).displayHardware());
    HWComposer& hwc(hw.getHwComposer());
	if((request == 9)||((Last2XScaleValue==9)&&(request==0)))
	{
		preProcessWithFrameBuffer( dirty,0.75f,0.75f);
		drawVideoHole( holex*0.75f,holey*0.75f,holew*0.75f,holeh*0.75f );
	}
	else if((request == 8)||((Last2XScaleValue==8)&&(request==0)))
	{
		float scale_x= (float)960/windowWidth;
		float scale_y= (float)1080/windowHeight;
		preProcessWithFrameBuffer( dirty,scale_x,scale_y);
       if(iptv != 8)
		drawVideoHole( holex*scale_x,holey*scale_y,holew*scale_x,holeh*scale_y );
	}
	else if((request == 16)||((Last2XScaleValue==16)&&(request==0)))
	{
		char scalest[8];
		float scale_x=0.0f;
		float scale_y=0.0f;
		int tmp= request_width*1000/windowWidth;//hw.getWidth();
		scale_x=(float)tmp/1000;
		tmp=request_height*1000/windowHeight;//hw.getHeight();
	...
	else if((request == 7)||((Last2XScaleValue==7)&&(request==0)))
	{
		char scalest[8];
		float scale_x=0.0f;
		float scale_y=0.0f;
       ...
       if(iptv != 8)
       	{
		drawVideoHole( holex*scale_x,holey*scale_y,holew*scale_x,holeh*scale_y );
		LOGE("Alex.cao composeSurfaces drawVideoHole re16 %d,%d,%d,%d\n",
			  holex*scale_x,holey*scale_y,holew*scale_x,holeh*scale_y);
       	}
	}
	else
	{
	    ...
	    for (size_t i=0 ; i<count ; i++) {
			...
			if(i==holelayer)
			{
				drawVideoHole( holex,holey,holew,holeh );
			}
	    }		
	}
	...
	if(request==1)
	{
		...
		snprintf(buf, SYSCMD_BUFSIZE, "%d %d %d %d", 0, 0,  hw.getWidth()/2-1,  hw.getHeight()-1);
	...
	else if(request==9)
	{
		bzero(buf, SYSCMD_BUFSIZE);
		//need change
		snprintf(buf, SYSCMD_BUFSIZE, "%d %d %d %d", 0, 0,  hw.getWidth()*3/4-1,  hw.getHeight()*3/4-1);
		amsysfs_set_sysfs_str(SCALE_AXIS_PATH,buf);
		
        char m1080scale;
        char output_mode[SYSCMD_BUFSIZE];
        char property[PROPERTY_VALUE_MAX];
        int enforce_progressive = 0;
        property_get("ro.platform.has.1080scale",&m1080scale, "0") ;
        ret = amsysfs_get_sysfs_str(OUTPUT_MODE_PATH, output_mode, SYSCMD_BUFSIZE);
		
        if (property_get("ro.build.product", property, NULL) > 0)
        {
            if ((strcmp(property, "c03ref") == 0)&&(!strncmp(output_mode, "1080i", 5))&&(m1080scale == '1'||m1080scale =='2'))
                enforce_progressive = 1;
            else
                enforce_progressive = 0;
        }
        if(enforce_progressive){
		 	LOGI("output mode =1080i, osd enforce progressive \n");   
		 	amsysfs_set_sysfs_str(ENFORCE_PROGRESSIVE_PATH,"1");	
			amsysfs_set_sysfs_str(SCALE_PATH,"0x10000");	
        }else{
            amsysfs_set_sysfs_str(SCALE_PATH,"0x10001");		
        }
	}	
	else if(request==8)
	{
		bzero(buf, SYSCMD_BUFSIZE);
		//need change
		snprintf(buf, SYSCMD_BUFSIZE, "%d %d %d %d", 0, 0,  959,  1079);
		amsysfs_set_sysfs_str(SCALE_AXIS_PATH,buf);
		amsysfs_set_sysfs_str(SCALE_PATH,"0x10000");	
...
void SurfaceFlinger::drawWormhole() const
{
    ...
    const DisplayHardware& hw(graphicPlane(0).displayHardware());
    const int32_t width = hw.getWidth(); //windowWidth;
    const int32_t height =hw.getHeight(); //windowHeight; 
...
{
    //LOGI("freezeDisplay (dpy=%d, flags=%d)", int(dpy), int(flags));
...
{
    //LOGI("unfreezeDisplay (dpy=%d, flags=%d)", int(dpy), int(flags));
        ...
        const layer_state_t& s)
{
    ...
    if (layer != 0) {
		...
		if (what & e3DFormatChanged ) {
            if (layer->set3DFormat(s.a3dformat))
                flags |= eTraversalNeeded;
        }
    }
    return flags;
...
void SurfaceFlinger::screenAcquired(int dpy)
{
    ...
    //work round for m3, flush screen again
    class MessageFlushScreen : public MessageBase {
        SurfaceFlinger* mFlinger;
    public:
        MessageFlushScreen(SurfaceFlinger* flinger)
            : mFlinger(flinger){}

        virtual bool handler() {
            Mutex::Autolock _l(mFlinger->mStateLock);

            LOGE("work round for m3 draw again");
            
            const DisplayHardware& hw(mFlinger->graphicPlane(0).displayHardware());
            mFlinger->mDirtyRegion.set(hw.bounds()); // careful that's not thread-safe 

            return false;
        }
    };

    sp<MessageBase> msg = new MessageFlushScreen(this);
    msg->what = MessageQueue::INVALIDATE;
    postMessageAsync(msg, ms2ns(50));
}
...
{
//	LOGW("scale %f %f",scale_x,scale_y);
        ...
        GLuint* textureName, GLfloat* uOut, GLfloat* vOut)
{
    ...
    const uint32_t hw_w = windowWidth; //hw.getWidth();
    const uint32_t hw_h = windowHeight;//hw.getHeight();
    GLfloat u = 1;
    GLfloat v = 1;
	float offsety=float(hw.getHeight()-windowHeight);
...
status_t SurfaceFlinger::renderScreenToTextureLocked2(DisplayID dpy,
        GLuint* textureName, GLfloat* uOut, GLfloat* vOut)
{
    if (!GLExtensions::getInstance().haveFramebufferObject())
        return INVALID_OPERATION;
	...
	int ret = amsysfs_get_sysfs_str(REQUEST_VIDEO_HOLE, videoholebuf, SYSCMD_BUFSIZE);
	if (ret < 0) {
		holelayer=0;
	}
	else{
		...
		if(holelayer<=0)holelayer=atoi(property_tmp);
	}
    ...
    for (size_t i=0 ; i<count ; ++i) {
        const sp<LayerBase>& layer(layers[i]);
        layer->drawForSreenShot();
		if(i==holelayer && (iptv == 8 || iptv == 16)&&(count != 1))
			{
				drawVideoHole( holex,holey,holew,holeh );
			}
    }
        ...
        uint32_t minLayerZ, uint32_t maxLayerZ)
{
    ...
    if ((sw > windowWidth) || (sh > windowHeight))
        return BAD_VALUE;

    sw = (!sw) ? windowWidth : sw;
    sh = (!sh) ? windowHeight : sh;
    ...
    if (status == GL_FRAMEBUFFER_COMPLETE_OES) {
        ...
        glOrthof(0, windowWidth, hw_h, hw_h-windowHeight, 0, 1);
        glMatrixMode(GL_MODELVIEW);
