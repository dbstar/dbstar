...
bool IsManifestUrl( const char* url) {
    bool ret=false;
    int len = strlen(url);
    //ALOGE("IsManifestUrl=%s",url);
    char * ManifestUrl = (char *)malloc(len+ 10);
    if(ManifestUrl){
        memset(ManifestUrl,0,len+10);
        memcpy(ManifestUrl,url,len);
        AString lowerCaseUrl(ManifestUrl);
        lowerCaseUrl.tolower();
        if (lowerCaseUrl.find("/manifest", 0) > 0) {  
            ret= true;// SmoothStreaming source detected.
            free(ManifestUrl);
        } 
    }
    return  ret;
        ...
        if (enableOSDVideo) {
            ...
            if (CallingAPkName[0] != '\0') {
                if (strcasestr(CallingAPkName, ".chrome")//chrome browser. //.android.chrome
                    ...
                    || strcasestr(CallingAPkName, ".opera.browser") //browser
                    ) 
                {
