...
public class SettingsVP {
	...
	public static boolean setVideoLayoutMode() {
		...
		try {
			BufferedReader in = new BufferedReader(new FileReader(
					displaymode_path), 32);
			BufferedReader in_axis = new BufferedReader(new FileReader(
					displayaxis_path), 32);
			try {
				...
				if (dispMode == null || dispaxis == null) {
	...
	public static boolean chkEnableOSD2XScale() {
		...
		if (temp_scale.equals("ok")) {
			/*
			 * String tmp_output =
			 * SystemProperties.get("ubootenv.var.outputmode");
			 * if(tmp_output.equals("1080p")) enable = true;
			 */
			String dispMode = null;
			File file = new File(displaymode_path);
			if (!file.exists()) {
				...
				try {
					...
					if (dispMode == null)
						enable = false;
							...
							|| dispMode.equals("lvds1080p")
							|| dispMode.equals("lvds1080p50hz"))
						enable = true;
				} finally {
					in.close();
				}
