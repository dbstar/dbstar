...
public class PlayerMenu extends Activity {
	...
	private enum Mode_3D {
		MODE_3D_DISABLE, MODE_3D_AUTO, MODE_3D_LR, MODE_3D_BT, MODE_3D_TO_2D_L, MODE_3D_TO_2D_R, MODE_3D_TO_2D_T, MODE_3D_TO_2D_B, MODE_3D_TO_2D_AUTO_1, MODE_3D_TO_2D_AUTO_2, MODE_2D_TO_3D, MODE_FIELD_DEPTH, MODE_3D_AUTO_SWITCH, MODE_3D_LR_SWITCH, MODE_3D_BT_SWITCH, MODE_3D_FULL_OFF, MODE_3D_LR_FULL, MODE_3D_BT_FULL, MODE_3D_GRATING_ON, MODE_3D_GRATING_OFF,

	}

	private int[] string_3d_id = { R.string.setting_3d_diable,
			R.string.setting_3d_auto, R.string.setting_3d_lr,
			R.string.setting_3d_bt, R.string.setting_3d_2d_l,
			R.string.setting_3d_2d_r, R.string.setting_3d_2d_t,
			R.string.setting_3d_2d_b, R.string.setting_3d_2d_auto1,
			R.string.setting_3d_2d_auto2, R.string.setting_2d_3d,
			R.string.setting_3d_field_depth, R.string.setting_3d_auto_switch,
			R.string.setting_3d_lr_switch, R.string.setting_3d_tb_switch,
			R.string.setting_3d_full_off, R.string.setting_3d_lr_full,
			R.string.setting_3d_tb_full, R.string.setting_3d_grating_open,
	...
	private SimpleAdapter getMorebarListAdapter(int id, int pos) {
		return new SimpleAdapter(this, getMorebarListData(id, pos),
				R.layout.list_row, new String[] { "item_name", "item_sel" },
	...
	private List<? extends Map<String, ?>> getMorebarListData(int id, int pos) {
		...
		case PLAY_MODE:
			...
			map.put("item_name",
					getResources().getString(
							R.string.setting_playmode_repeatall));
			map.put("item_sel", R.drawable.item_img_unsel);
			list.add(map);
			map = new HashMap<String, Object>();
			map.put("item_name",
					getResources().getString(
							R.string.setting_playmode_repeatone));
			map.put("item_sel", R.drawable.item_img_unsel);
			list.add(map);
		...
		case AUDIOTRACK:
			if (AudioTrackOperation.AudioStreamFormat.size() < bMediaInfo
					.getAudioTrackCount())
			...
			if (size_as > 0) {
				for (int i = 0; i < size_as; i++) {
					if (AudioTrackOperation.AudioStreamFormat.get(i) != "UNSUPPORT") {
						map = new HashMap<String, Object>();
						map.put("item_name",
								AudioTrackOperation.AudioStreamFormat.get(i));
						map.put("item_sel", R.drawable.item_img_unsel);
						list.add(map);
					}
				}
		...
		case DISPLAY:
			...
			map.put("item_name",
					getResources().getString(
							R.string.setting_displaymode_normal));
			map.put("item_sel", R.drawable.item_img_unsel);
			list.add(map);
			map = new HashMap<String, Object>();
			map.put("item_name",
					getString(R.string.setting_displaymode_fullscreen));
			map.put("item_sel", R.drawable.item_img_unsel);
			list.add(map);
			map = new HashMap<String, Object>();
			...
			if (SystemProperties.getBoolean("3D_setting.enable", false)) {
				if (is3DVideoDisplayFlag) {// judge is 3D
					...
					map.put("item_name",
							getResources()
									.getString(
											R.string.setting_displaymode_normal_noscaleup));
					map.put("item_sel", R.drawable.item_img_unsel);
					list.add(map);
				}
			}
	...
	private boolean isSubtitleOn() {
		if (sub_para != null && sub_para.totalnum > 0
	...
	void setOSDOnOff(boolean on) {
		...
		if (!on && !mPaused) {
			if (isSubtitleOn()) {
				...
				if (ori == 90)
					writeSysfs(OSD_BLOCK_MODE_PATH, "0x20001"); // OSD ver blk0
																// enable
				else if (ori == 180)
					writeSysfs(OSD_BLOCK_MODE_PATH, "0x10001"); // OSD hor blk0
																// enable
				else if (ori == 270)
					writeSysfs(OSD_BLOCK_MODE_PATH, "0x20008"); // OSD ver blk3
																// enable
				else
					writeSysfs(OSD_BLOCK_MODE_PATH, "0x10008"); // OSD hor blk3
																// enable
	...
	private void videobar() {
		...
		if (AmPlayer.getProductType() == 1) {
			if (SettingsVP.display_mode.equals("480p")
				...
				FrameLayout.LayoutParams frameParams = (FrameLayout.LayoutParams) baselayout2
						.getLayoutParams();
		...
		resume.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				listView.setAdapter(getMorebarListAdapter(PLAY_RESUME,
						SettingsVP.getParaBoolean(SettingsVP.RESUME_MODE) ? 0
								: 1));
				listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					public void onItemClick(AdapterView<?> parent, View view,
							int position, long id) {
						if (position == 0)
							SettingsVP.putParaBoolean(SettingsVP.RESUME_MODE,
									true);
						else if (position == 1)
							SettingsVP.putParaBoolean(SettingsVP.RESUME_MODE,
									false);
		...
		if (playmode_switch) {
			playmode.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					...
					ListView listView = (ListView) findViewById(R.id.AudioListView);
					listView.setAdapter(getMorebarListAdapter(PLAY_MODE,
							m_playmode - 1));
					listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			playmode.setImageDrawable(getResources().getDrawable(
					R.drawable.mode_disable));
		}
					...
					listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
						public void onItemClick(AdapterView<?> parent,
							...
							case 0:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_DISABLE
													.ordinal());
							...
							case 1:
								try {
									m_Amplayer.Set3Dmode(Mode_3D.MODE_3D_AUTO
											.ordinal());
							...
							case 2:
								try {
									m_Amplayer.Set3Dmode(Mode_3D.MODE_3D_LR
											.ordinal());
									is3DVideoDisplayFlag = true;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 3:
								try {
									m_Amplayer.Set3Dmode(Mode_3D.MODE_3D_BT
											.ordinal());
									is3DVideoDisplayFlag = true;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 4:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_TO_2D_L
													.ordinal());
									is3DVideoDisplayFlag = false;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 5:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_TO_2D_R
													.ordinal());
									is3DVideoDisplayFlag = false;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 6:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_TO_2D_T
													.ordinal());
									is3DVideoDisplayFlag = false;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 7:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_TO_2D_B
													.ordinal());
									is3DVideoDisplayFlag = false;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 8:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_TO_2D_AUTO_1
													.ordinal());
									is3DVideoDisplayFlag = false;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 9:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_TO_2D_AUTO_2
													.ordinal());
									is3DVideoDisplayFlag = false;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 10:
								try {
									m_Amplayer.Set3Dmode(Mode_3D.MODE_2D_TO_3D
											.ordinal());
									is3DVideoDisplayFlag = true;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 11:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_FIELD_DEPTH
													.ordinal());
									is3DVideoDisplayFlag = true;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 12:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_AUTO_SWITCH
													.ordinal());
									is3DVideoDisplayFlag = true;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 13:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_LR_SWITCH
													.ordinal());
									is3DVideoDisplayFlag = true;
								} catch (RemoteException e) {
									e.printStackTrace();
							...
							case 14:
								try {
									m_Amplayer
											.Set3Dmode(Mode_3D.MODE_3D_BT_SWITCH
													.ordinal());
									is3DVideoDisplayFlag = true;
								} catch (RemoteException e) {
									e.printStackTrace();
		...
		audiotrack.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				SimpleAdapter audioarray = getMorebarListAdapter(AUDIOTRACK,
						cur_audio_stream);
				if ((audio_flag == Errorno.PLAYER_NO_AUDIO)
						|| (audioarray.getCount() <= 0)) {
					Toast toast = Toast.makeText(PlayerMenu.this,
							R.string.file_have_no_audio, Toast.LENGTH_SHORT);
				...
				listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					public void onItemClick(AdapterView<?> arg0, View arg1,
						...
						if (bMediaInfo.getAudioTrackCount() > 1) {
							try {
								m_Amplayer
										.SwitchAID(AudioTrackOperation.AudioStreamInfo
												.get(arg2).audio_id);
								Log.d("audiostream", "change audio stream to: "
										+ arg2);
		...
		subtitle.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (sub_para.totalnum <= 0) {
					Toast toast = Toast.makeText(PlayerMenu.this,
							R.string.sub_no_subtitle, Toast.LENGTH_SHORT);
					toast.setGravity(Gravity.BOTTOM, 110, 0);
					toast.setDuration(0x00000001);
					toast.show();
					waitForHideOsd();
			...
			String color_text[] = {
					PlayerMenu.this.getResources().getString(
			...
			private void subtitle_control() {
				...
				ok.setOnClickListener(new View.OnClickListener() {
					public void onClick(View v) {
						...
						if (sub_para.curid == sub_para.totalnum) {
							...
							sub_para.sub_id = subtitleUtils
									.getSubID(sub_para.curid);
						}
				...
				Bswitch_l.setOnClickListener(new View.OnClickListener() {
					public void onClick(View v) {
						...
						if (sub_switch_state == sub_para.totalnum)
							...
							t_subswitch.setText(String
									.valueOf(sub_switch_state + 1)
									+ "/"
									+ String.valueOf(sub_para.totalnum));
					}
				});
				...
				Bposition_v_l.setOnClickListener(new View.OnClickListener() {
					public void onClick(View v) {
						...
						t_subsposition_v.setText(String
								.valueOf(sub_position_v_state));
					}
				});
				...
				if (sub_para.sub_id != null) {
					if (sub_para.sub_id.filename.equals("INSUB")
							|| sub_para.sub_id.filename.endsWith(".idx")) {
						disableSubSetOptions();
					}
				}
			}
		...
		display.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if (is3DVideoDisplayFlag) {
					listView.setAdapter(getMorebarListAdapter(DISPLAY,
							view_mode));
				} else {
					listView.setAdapter(getMorebarListAdapter(DISPLAY,
				...
				listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
					public void onItemClick(AdapterView<?> parent, View view,
							int position, long id) {
						// if(SystemProperties.getBoolean("3D_setting.enable",
						// false)){
						if (is3DVideoDisplayFlag) {
							if (is3DVideoDisplayFlag) {// judge is 3D
								view_mode = position;
							...
							case ScreenMode.NORMAL:
								SettingsVP.putParaInt(SettingsVP.DISPLAY_MODE,
										ScreenMode.NORMAL);
								ScreenMode.setScreenMode("0");
								break;
							case ScreenMode.FULLSTRETCH:
								SettingsVP.putParaInt(SettingsVP.DISPLAY_MODE,
										ScreenMode.FULLSTRETCH);
							...
							case ScreenMode.RATIO16_9:
								SettingsVP.putParaInt(SettingsVP.DISPLAY_MODE,
										ScreenMode.RATIO16_9);
								ScreenMode.setScreenMode("3");
								break;
							/*
							 * case ScreenMode.NORMAL_NOSCALEUP:
							 * SettingsVP.putParaInt(SettingsVP.DISPLAY_MODE,
							 * ScreenMode.NORMAL_NOSCALEUP);
							 * ScreenMode.setScreenMode("4"); break;
							 */
							default:
								break;
							}
				...
				public void onClick(View v) {
					...
					try {
						mBrightness = Settings.System.getInt(
								PlayerMenu.this.getContentResolver(),
						...
						public void onItemClick(AdapterView<?> parent,
							...
							try {
								...
								if (power != null) {
									...
									Settings.System.putInt(PlayerMenu.this
											.getContentResolver(),
											Settings.System.SCREEN_BRIGHTNESS,
											brightness);
								}
...
//			}
			...
			if (OSD_MORE_BAR == flag) {
				if ((otherbar.getVisibility() == View.VISIBLE)
					...
					if (SystemProperties.getBoolean(
							"ro.platform.has.mbxuimode", false)) {
				...
				item_position_selected = item_position_selected_init
						+ PlayList.getinstance().getindex();
		 ...
		 * subtitle = (ImageButton) findViewById(R.id.SubtitleBtn);
		 * subtitle.requestFocusFromTouch(); return (true); } else if (keyCode
		 * == KeyEvent.KEYCODE_MEDIA_AUDIO) { videobar(); ImageButton subtitle =
		 * (ImageButton) findViewById(R.id.ChangetrackBtn);
		 * subtitle.requestFocusFromTouch(); return (true); }
		 */
		else
...
//							startActivity(selectFileIntent);
		...
		if (it.getData() != null) {
			if (it.getData().getScheme().equals("file")) {
				...
				Cursor cursor = managedQuery(it.getData(), null, null, null,
						null);
				cursor.moveToFirst();
				...
				if ((index == -1) || (cursor.getCount() <= 0)) {
					...
					// Log.d(TAG, "From content providor DATA:" +
		...
		if (m1080scale == 2) { // set video position for MBX
			Intent intent_videoposition_change = new Intent(
		...
		mReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				if (mHdmiPlugged != plugged && confirm_dialog != null
						&& confirm_dialog.isShowing()) {
		...
		try {
			...
			if (bundle != null) {
				...
				if (item_init_flag) {
					item_position_selected_init = item_position_selected
							- PlayList.getinstance().getindex();
			...
			if (dirFile.exists() && dirFile.isDirectory()
				...
				for (File file : dirFile.listFiles()) {
					...
					if (ext.equals("rm") || ext.equals("rmvb")
							...
							|| ext.equals("mp4") || ext.equals("wmv")
							|| ext.equals("mov") || ext.equals("flv")
							|| ext.equals("asf") || ext.equals("3gp")
							|| ext.equals("mpg") || ext.equals("m2ts")
							|| ext.equals("ts") || ext.equals("swf")
							|| ext.equals("divx") || ext.equals("3gp2")
							|| ext.equals("3gpp") || ext.equals("m4v")
							|| ext.equals("mts") || ext.equals("tp")
							|| ext.equals("webm") || ext.equals("3g2")
							|| ext.equals("f4v") || ext.equals("mpeg")
							|| ext.equals("vob") || ext.equals("dat")) {
						list.add(pathFull);
					}
				}
			}
	...
	private void displayinit() {
		...
		case ScreenMode.RATIO16_9:
			...
			break;
		/*
		 * case ScreenMode.NORMAL_NOSCALEUP: ScreenMode.setScreenMode("4");
		 * break;
		 */
		default:
	...
	protected void initinfobar() {
		...
		if (AmPlayer.getProductType() == 1) {
			if (SettingsVP.display_mode.equals("480p")
					&& SettingsVP.panel_height > 480) {
				linearParams = (LinearLayout.LayoutParams) subTitleView
						.getLayoutParams();
				if (SettingsVP.panel_width > 720)
		...
		if (AmPlayer.getProductType() == 1) {
			if (SettingsVP.display_mode.equals("480p")
					&& SettingsVP.panel_height > 480) {
				linearParams = (LinearLayout.LayoutParams) infobar
						.getLayoutParams();
				if (SettingsVP.panel_width > 720)
					linearParams.width = 720;
				linearParams.bottomMargin = SettingsVP.panel_height - 480 + 10;
		...
		browser.setOnClickListener(new ImageButton.OnClickListener() {
			public void onClick(View v) {
				// TODO Auto-generated method stub
				/*
				 * hui.xu remove if(bMediaInfo == null) return;
				 */
				if (SettingsVP.chkEnableOSD2XScale() == true) {
					if (infobar != null) {
						infobar.setVisibility(View.GONE);
		...
		preItem.setOnClickListener(new Button.OnClickListener() {
			public void onClick(View v) {
				...
				if (!INITOK)
					return;
				isBackWard = true;
				ResumePlay.saveResumePara(PlayList.getinstance().getcur(),
						curtime);
				String filename = PlayList.getinstance().moveprev();
				toast.cancel();
			...
			public void handleMessage(Message msg) {
				...
				case 0x3d:
					if (confirm_dialog.isShowing()) {
						if (resumeSecond > 0) {
							String cancel = PlayerMenu.this.getResources()
									.getString(R.string.str_cancel);
							confirm_dialog.getButton(
									AlertDialog.BUTTON_NEGATIVE).setText(
									cancel + " ( " + (--resumeSecond) + " )");
							ResumeCountdown();
						} else {
							playPosition = 0;
	...
	public void onPause() {
		...
		setOSDOnOff(true);
		// StorageManager m_storagemgr = (StorageManager)
		...
		public void handleMessage(Message msg) {
			...
			case VideoInfo.TIME_INFO_MSG:
				...
				if (player_status == VideoInfo.PLAYER_RUNNING) {
					...
					if (SystemProperties.getBoolean("3D_setting.enable", false)) {
						if (subTitleView_sm != null
								& View.INVISIBLE == subTitleView_sm
										.getVisibility()
								&& is3DVideoDisplayFlag) {
				...
				if (totaltime == 0)
					...
					// if(!progressSliding)
					myProgressBar
							.setProgress(msg.arg1 / 1000 * 100 / totaltime);
				}
				break;
			case VideoInfo.STATUS_CHANGED_INFO_MSG:
				...
				case VideoInfo.PLAYER_RUNNING:
					play.setImageResource(R.drawable.pause);
					String videoFotmat = bMediaInfo.getFullFileName(PlayList
							.getinstance().getcur());
				...
				case VideoInfo.PLAYER_EXIT:
					...
					if (PRE_NEXT_FLAG == 1 && !backToFileList) {
						...
						Log.d(TAG, "PRE_NEXT_FLAG == " + PRE_NEXT_FLAG
								+ "backToFileList==" + backToFileList);
					...
					if (subTitleView != null) {
						...
						if (subTitleView_sm != null
								&& SystemProperties.getBoolean(
										"3D_setting.enable", false)) {
							subTitleView_sm.closeSubtitle(); // need return
																// focus.
				...
				case VideoInfo.PLAYER_ERROR:
					...
					if (msg.arg2 < 0) {
						...
						ResumePlay.saveResumePara(PlayList.getinstance()
								.getcur(), 0);
						playPosition = 0;
						if (m_playmode == REPEATLIST)
							if (isBackWard) {
				...
				case VideoInfo.PLAYER_INITOK:
					...
					if ((bMediaInfo != null) && (subTitleView != null)) {
						...
						subTitleView.setVideoResolution(bMediaInfo.getWidth(),
								bMediaInfo.getHeight());
					}
					if (bMediaInfo != null
							&& subTitleView_sm != null
							&& SystemProperties.getBoolean("3D_setting.enable",
					...
					if (SystemProperties.getBoolean("3D_setting.enable", false)
							&& bMediaInfo.getVideoFormat().compareToIgnoreCase(
					...
					sub_para.totalnum = subtitleUtils.getExSubTotal()
							+ InternalSubtitleInfo.getInsubNum();
					if (sub_para.totalnum > 0) {

						sub_para.curid = subtitleUtils
	...
	public IPlayerService m_Amplayer = null;
	...
	private void Amplayer_play() {
		...
		try {
			...
			if (SystemProperties.getBoolean("3D_setting.enable", false)) {
				...
				if (PlayList.getinstance().getcur().indexOf("[3D]") != -1
						&& PlayList.getinstance().getcur().indexOf("[HALF]") != -1) {
					m_Amplayer.Set3Dgrating(1);
					m_Amplayer.Set3Dmode(1);
					mode_3d = 1;
					is3DVideoDisplayFlag = true;
				} else if (PlayList.getinstance().getcur().indexOf("[3D]") != -1
						&& PlayList.getinstance().getcur().indexOf("[FULL]") != -1) {
					m_Amplayer.Set3Dgrating(1);
	...
	private void Amplayer_stop() {
		...
		if (SystemProperties.getBoolean("ro.video.deinterlace.enable", false)) {
			if (bMediaInfo != null
					&& bMediaInfo.getWidth() * bMediaInfo.getHeight() < 1280 * 720) {
	...
	ServiceConnection m_PlayerConn = new ServiceConnection() {
		public void onServiceConnected(ComponentName name, IBinder service) {
			m_Amplayer = IPlayerService.Stub.asInterface(service);
	...
	public void StartPlayerService() {
		Intent intent = new Intent();
		ComponentName hcomponet = new ComponentName("com.dbstar.DbstarDVB",
	...
	private class myAlertDialogDismiss implements
	...
	// return;
	//
	// if(newState.compareTo("unmounted") == 0||newState.compareTo("removed") ==
	// 0) {
	...
	// if(subTitleView!=null)
	// subTitleView.closeSubtitle();
	// if(subTitleView_sm!=null&&SystemProperties.getBoolean("3D_setting.enable",
	// false)){
	// subTitleView_sm.closeSubtitle();
	// }
	//
...
//			dialog = mVideoInfoDlg;
	...
	private int firstInvokFlag = 1;// tony.wang 20120601 add for box shield
									// scale interface on start of playing
	...
	private void showInfoBar() {
		if ((null != infobar) && (View.GONE == infobar.getVisibility())) {
			...
			if ((null != subbar) && (View.VISIBLE == subbar.getVisibility()))
				subbar.setVisibility(View.GONE);
			if ((null != otherbar)
					&& (View.VISIBLE == otherbar.getVisibility()))
				otherbar.setVisibility(View.GONE);
			if ((null != infodialog)
					&& (View.VISIBLE == infodialog.getVisibility()))
				infodialog.setVisibility(View.GONE);
	...
	private void showOsdView() {
		...
		default: {
			Log.e(TAG, "[showOsdView]getCurOsdView error flag:" + flag
					+ ",set CurOsdView default");
			showInfoBar();
		}
			break;
		}
	...
	private void switchOsdView() {
		...
		default: {
			Log.e(TAG, "[switchOsdView]getCurOsdView error flag:" + flag
					+ ",set CurOsdView default");
			showInfoBar();
		}
			break;
	...
	private void setSubtitleView() {
		...
		subTitleView.setPadding(subTitleView.getPaddingLeft(),
				subTitleView.getPaddingTop(), subTitleView.getPaddingRight(),
				getWindowManager().getDefaultDisplay().getRawHeight()
						* sub_para.position_v / 20 + 10);
	}
	...
	private void initSubSetOptions(String color_text[]) {
		...
		if (sub_para.curid == sub_para.totalnum || sub_para.enable == false) {
			...
			t_subswitch.setText(String.valueOf(sub_para.curid + 1) + "/"
					+ String.valueOf(sub_para.totalnum));
