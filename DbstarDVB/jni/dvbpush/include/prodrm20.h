...
/* PVOD授权信息 */
typedef struct {
    CDCA_U16   m_OperatorID;     /* 运营商ID */ //gyw add 2011-07-29
    CDCA_U32   m_ID;              /* 授权的产品ID，0表示无效或者没有 */
    CDCA_TIME  m_ProductStartTime;  /* 发行开始时间，注：0表示在终端购买时，由用户输入开始时间  */
    CDCA_TIME  m_ProductEndTime;    /* 发行结束时间，注：由用户输入开始时间时，此值表示时间周期 */
                                  /* 当产品是节目包时，发行时间范围决定了属于这个产品的哪些节目允许用户观看 */

    CDCA_TIME  m_WatchStartTime;  /* 观看开始时间，注：0表示在终端购买时，由用户输入开始时间  */
    CDCA_TIME  m_WatchEndTime;    /* 观看结束时间，注：由用户输入开始时间时，此值表示时间周期 */
    CDCA_U32   m_LimitTotaltValue;/* 限量总值，0表示不限量 */
    CDCA_U32   m_LimitUsedValue;  /* 限量已用值 */
...
typedef struct {
    CDCA_U32   m_ID;             /* 产品ID */
    CDCA_TIME  m_IssueStartTime; /* 在此产品内的发行开始时间 */
    CDCA_TIME  m_IssueEndTime;   /* 在此产品内的发行结束时间 */
}SCDCAPVODPackInfo;


/* PVOD节目信息 */
typedef struct {
    CDCA_U32          m_ID;         /* 节目内容ID     */
    CDCA_U16          m_OperatorID; /* 运营商ID   */
    CDCA_U8            m_Reserved;   /* 保留       */
    CDCA_U8            m_PackNum;    /* 节目包个数 */
    SCDCAPVODPackInfo  m_Packs[CDCA_MAXNUM_PVOD_ITEM];      /* 节目包信息 */
}SCDCAPVODProgramInfo;
...
/* 获取回传数据 */
extern CDCA_U16 CDCASTB_DRM_GetCallBack(CDCA_U8* pbyBuff, CDCA_U16* pwBuffLen);

/* 头端回应数据*/
extern void CDCASTB_DRM_CallBackResponse(const CDCA_U8* pbyBuff, CDCA_U16 wBuffLen);
...
/* 查询节目信息*/
extern CDCA_U16 CDCASTB_DRM_GetProgramInfo(const void*              pFileHnd,
                                    CDCA_U32*             pdwFrom,
                                    SCDCAPVODProgramInfo* pProgramInfo,
                                    CDCA_U32*             pdwNum);
...
/* 查询授权信息 */
extern CDCA_U16  CDCASTB_DRM_GetEntitleInfo(CDCA_U32*              pdwFrom,
                                     SCDCAPVODEntitleInfo*  pEntitleInfo, 
                                     CDCA_U32*              pdwNum);
...
/* 打开节目文件 */
extern CDCA_U16 CDCASTB_DRM_OpenFile(const void*    pDataFileHnd, 
                                     const void*    pDrmFileHnd);
...
/* 读取文件数据 */
extern CDCA_U16 CDCASTB_DRM_ReadFile(const void*    pDataFileHnd,
                                     CDCA_U8*       pbyBuffer,
                                     CDCA_U32*      pdwBufferLen);


/* 文件定位 */
extern CDCA_BOOL CDCASTB_DRM_SeekFilePos(const void*  pDataFileHnd,
                                  CDCA_U32     dwDataPosK,
                                  CDCA_U32     dwDataPos);
...
/* 导出授权数据 */
extern CDCA_U16 CDCASTB_DRM_ExportEntitleFile(char      CardSN[CDCA_MAXLEN_SN+1],
                                       const void*     pFileHandle);

/* 同步授权到智能卡 */
extern CDCA_U16 CDCASTB_DRM_SyncEntitleToCard(void);
...
/* 回传数据通知*/
extern void CDSTBCA_NotifyCallBack(void);
...
/* 打开授权文件 */
extern CDCA_BOOL CDSTBCA_DRM_OpenEntitleFile(char   CardSN[CDCA_MAXLEN_SN+1],
                                      void** pFileHandle);
...
/* 移动文件指针*/
extern CDCA_BOOL CDSTBCA_SeekPos(const void* pFileHandle, 
                                 CDCA_U8     byOrigin, 
...
/* 读文件 */
extern CDCA_U32 CDSTBCA_ReadFile(const void* pFileHandle, CDCA_U8* pBuf, CDCA_U32 dwLen);
