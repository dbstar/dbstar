...
#define MAX_PACK_BUF (40000)		//定义缓冲区大小，单位：包
...
母盘解析时，arg_ext为空，导致试图删除/mnt/sda1
	if(COLUMN_XML==push_flag){
		...
		remove_force(__FUNCTION__, reject_uri);
...
void *maintenance_thread()
{
	...
	time_t last_checkreboot_sec = time(NULL);	// 上次检查是否重启的时间
	time_t now_sec;
	struct tm now_tm;
	char sqlite_cmd[256];
	int ret = 0;
	...
	//  && 1==network_init_status()	// 省略这个判断可能对厂测流程有影响，尤其是当插上母盘进行厂测时容易导致母盘数据混乱
...
// push regist一定和母盘初始化在一个线程中，确保不并行，避免push接收和母盘初始化同时进行导致数据库混乱
		push_regist_init();
	}
...
#if 1
// 增加AP功能后，终端不能频繁自动重启，暂定在每周五凌晨新播发单生效前一小时
// 当前的重启时间就不需要考虑国电了。每隔20分钟检查一下。
...
//			DEBUG("in user idle status,now_sec: %ld, s_pin_sec=%ld, reboot_timestamp_get()=%d\n", now_sec, s_pin_sec, reboot_timestamp_get());
			...
			// 1、与上次检查间隔大于20分钟（1200）
			// 2、与上次重启的时间差大于12个小时(43200)
			// 3、开机超过12个小时(43200)；
			// 之所以有第2个判断，是因为有可能开机时间为1970年，很快接收到时间同步为2014年，引起意外满足开机大于12个小时
			if(		now_sec-last_checkreboot_sec>1200
				...
				// 废计划：国电网关需要在45分和整点之间保持开机状态，预留15分钟重启时间，窗口时间每小时为0分到30分
				//if(reboot_hour==now_tm.tm_hour && now_tm.tm_min>=0 && now_tm.tm_min<=30){
				
				// 现计划2014-09-29：不再考虑国电，根据播发单开始时间，只考虑在星期四凌晨或星期五凌晨。
				// 如果重启时间在0～7点，则在周五重启；其他时间点在周四重启
