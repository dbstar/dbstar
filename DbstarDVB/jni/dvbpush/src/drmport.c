...
#define BLOCK01_FILE "./entitle/block01"
#define ENTITLE_FILE_PATH "./entitle"
#define DMX_DEV_NO 0
...
{
        static int i = 0;
printf("DRM Cread a the [%d] thread of name [%s]\n",i,szName);
	if(pthread_create(&tmpthread[i], NULL, pTaskFun, pParam) != 0)
	{
		printf( "Create task [%s] ERROR\n",szName);
		return CDCA_FALSE;
	}
        i++;
        if(i >= MAX_DRM_THREADS) i = MAX_DRM_THREADS - 1;
	return CDCA_TRUE;
...
{
    if (bInitVal == CDCA_TRUE) 	*pSemaphore = 1;
    else *pSemaphore = 0;
...
if (card_sn[i].fd)  printf("open the entitle [%d] successful\n",i);
            	*pFileHandle = card_sn[i].fd;
                 //break;
            }
            break;
...
if(card_sn[0].fd) printf("open the entitle 0 successful\n");
        *pFileHandle = card_sn[0].fd;
    }
    
    if (*pFileHandle) return CDCA_TRUE;
...
if(card_sn.fd) printf("open the entitle 0 successful\n");
...
{
printf("seek the file ori[%d] pos [%d]k [%d] \n",byOrigin,dwOffsetKByte,dwOffsetByte);
...
printf("read file len [%d]\n",dwLen);
	if (!pFileHandle) 
		return -1;
	/*return*/ ret = fread(pBuf,1,dwLen,(FILE *)pFileHandle);
if (ret > 0)
