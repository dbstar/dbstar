...
static pthread_mutex_t mtx_chann_filter = PTHREAD_MUTEX_INITIALIZER;

static char software_version[16];

int TC_loader_filter_handle(int aof);
static void TC_free_filter_t(int fid);
#endif
...
#ifdef TUNER_INPUT
static int MX_alloc_filter_t(unsigned short pid, Filter_param* param, dataCb hdle, void* userdata, char priority)
#else
// _t function has no mutex lock
int TC_alloc_filter_t(unsigned short pid, Filter_param* param, dataCb hdle, void* userdata, char priority)
#endif
{
...
{
	return MX_alloc_filter_t(pid, param, hdle, userdata, priority);
}
#else
// this function has mutex lock
int TC_alloc_filter(unsigned short pid, Filter_param* param, dataCb hdle, void* userdata, char priority)
{
	...
	int ret = TC_alloc_filter_t(pid, param, hdle, userdata, priority);
...
patch0:
				...
				if (total_loader >= maxSeq)
				{
					...
					TC_free_filter_t(g_loaderInfo.fid);
                ...
                if (seq == 1)
                {
                    ...
                    if(total_cycle > 5)
                    {
                        ...
                        TC_loader_to_push_order(1);
                        {
	                        ...
	                        loader_dsc_fid=TC_alloc_filter_t(0x1ff0, &param, loader_des_section_handle, NULL, 1);
                        }
                    } 
                }
			}
...
*/
	...
	s_print_cnt = 0;
	TC_free_filter_t(loader_dsc_fid);
	//datap += 4;
	{
		//unsigned short pid;
		//unsigned char tid;
		...
		param.mask[0] = 0xff;
		g_loaderInfo.fid = TC_alloc_filter_t(tc_pid, &param, loader_section_handle, NULL, 1);
...
#ifdef TUNER_INPUT
static void MX_free_filter_t(int fid)
#else
static void TC_free_filter_t(int fid)
#endif
{
...
{
	return MX_free_filter_t(fid);
}
#else
void TC_free_filter(int fid)
{
	pthread_mutex_lock(&mtx_chann_filter);
	TC_free_filter_t(fid);
	pthread_mutex_unlock(&mtx_chann_filter);
	return;
}
#endif
...
int free_filter(unsigned short pid)
{
	...
	for(i=0; i < MAX_CHAN_FILTER; i++)
	{
		...
		{
#ifdef TUNER_INPUT
			MX_free_filter_t(chanFilter[i].fid);
#else
			TC_free_filter_t(chanFilter[i].fid);
#endif
			ret = 0;
			break;
		}
...
static int parse_ts_packet_t(unsigned char *ptr, int write_ptr, int *read)
{
	int p = *read;
