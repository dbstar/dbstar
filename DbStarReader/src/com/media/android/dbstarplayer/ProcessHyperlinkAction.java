...
import com.media.android.dbstarplayer.network.BookDownloader;
import com.media.android.dbstarplayer.network.BookDownloaderService;
import com.media.android.dbstarplayer.image.ImageViewActivity;

class ProcessHyperlinkAction extends DbStarAndroidAction {
	ProcessHyperlinkAction(DbStarPlayer baseActivity, DbStarPlayerApp dbstarplayerapp) {
	...
	private void openInBrowser(final String url) {
		...
		new Thread(new Runnable() {
			public void run() {
				if (!url.startsWith("dbstarplayer-action:")) {
					nLibrary.initialize();
				}
