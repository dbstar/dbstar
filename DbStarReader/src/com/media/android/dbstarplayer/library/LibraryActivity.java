...
import com.media.dbstarplayer.library.*;
...
import com.media.android.dbstarplayer.tree.TreeActivity;
...
public class LibraryActivity extends TreeActivity<LibraryTree> implements MenuItem.OnMenuItemClickListener, View.OnCreateContextMenuListener, IBookCollection.Listener {
	static final String START_SEARCH_ACTION = "action.dbstarplayer.library.start-search";
	...
	public void onCreate(Bundle icicle) {
		...
		mySelectedBook =
			SerializerUtil.deserializeBook(getIntent().getStringExtra(DbStarPlayer.BOOK_KEY));
	...
	private void showBookInfo(Book book) {
		OrientationUtil.startActivity(
			...
			new Intent(getApplicationContext(), BookInfoActivity.class)
				.putExtra(DbStarPlayer.BOOK_KEY, SerializerUtil.serialize(book))
		);
	}
	...
	private boolean onContextItemSelected(int itemId, Book book) {
		switch (itemId) {
			case OPEN_BOOK_ITEM_ID:
				DbStarPlayer.openBookActivity(this, book, null);
				return true;
			case SHOW_BOOK_INFO_ITEM_ID:
				showBookInfo(book);
				return true;
			case SHARE_BOOK_ITEM_ID:
				DbStarUtil.shareBook(this, book);
				return true;
