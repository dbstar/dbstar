...
import com.media.dbstarplayer.bookmodel.FBHyperlinkType;
...
import com.media.zlibrary.core.filesystem.ZLResourceFile;
...
import com.media.zlibrary.text.view.ZLTextHyperlinkRegionSoul;
import com.media.zlibrary.text.view.ZLTextImageRegionSoul;
import com.media.zlibrary.text.view.ZLTextRegion;
import com.media.zlibrary.text.view.ZLTextSelectionCursor;
...
public final class DbStarView extends ZLTextView {
	private DbStarPlayerApp myReader;

	DbStarView(DbStarPlayerApp reader) {
		super(reader);
		myReader = reader;
	}
...
//		}
		...
		if (region != null) {
			...
			if (soul instanceof ZLTextWordRegionSoul) {
				...
				doSelectRegion =
					...
					DbStarPlayerApp.ImageTappingAction.doNothing;
		...
		if (region != null) {
			...
			if (soul instanceof ZLTextHyperlinkRegionSoul ||
				...
				if (myReader.WordTappingActionOption.getValue() !=
					DbStarPlayerApp.WordTappingAction.doNothing) {
		...
		if (region != null) {
			...
			if (soul instanceof ZLTextWordRegionSoul) {
				doRunAction =
					...
					DbStarPlayerApp.WordTappingAction.openDictionary;
			} else if (soul instanceof ZLTextImageRegionSoul) {
				doRunAction =
					myReader.ImageTappingActionOption.getValue() ==
					DbStarPlayerApp.ImageTappingAction.openImageView;
			}
	...
	public int getTopMargin() {
		return myReader.TopMarginOption.getValue()+
				Utils.dip2px(((ZLAndroidLibrary) ZLAndroidLibrary.Instance()).
				getWidget().getContext(), HEIGHT_TITLE_DIP+MARGIN_TITLE_CONTENT_DIP);
	}
...
//			}
//
//			final DbStarPlayerApp reader = myReader;
//			if (reader == null) {
//				return;
//			}
...
//			);
//
//			final PagePosition pagePosition = DbStarView.this.pagePosition();
//
