...
import com.media.zlibrary.core.xml.*;

import com.media.dbstarplayer.network.atom.*;
...
public class OPDSXMLReader extends ATOMXMLReader<OPDSFeedMetadata, OPDSEntry> {
	...
	private static final int DBSTARPLAYER_VIEW = ATOM_STATE_FIRST_UNUSED + 11;
	...
	private static final String DBSTARPLAYER_TAG_VIEW = "view";
		...
		switch (myState) {
			case FEED:
				if (ns == XMLNamespaces.OpenSearch) {
					...
					return false;
				} else if (ns == XMLNamespaces.DbStarPlayerCatalogMetadata){
					if (tag == DBSTARPLAYER_TAG_VIEW) {
						myState = DBSTARPLAYER_VIEW;
					}
				} else {
	...
	public boolean endElementHandler(String ns, String tag, String bufferContent) {
		switch (myState) {
			...
			case OPENSEARCH_STARTINDEX:
				...
				return false;
			case DBSTARPLAYER_VIEW:
				if (ns == XMLNamespaces.DbStarPlayerCatalogMetadata && tag == DBSTARPLAYER_TAG_VIEW) {
					if (getOPDSFeed() != null) {
