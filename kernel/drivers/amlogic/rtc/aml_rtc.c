...
static unsigned long s_alarm_secs = 0;
...
static int aml_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
    ...
    printk("[RTC] %s(alarm_secs=%ld, cur_secs=%ld)\n", __func__, alarm_secs, cur_secs);
...
static int aml_rtc_suspend(struct platform_device *pdev, pm_message_t state)
{
    ...
    if (s_alarm_secs > cur_secs) {
        alarm_data.alarm_sec = s_alarm_secs - cur_secs;
        alarm_data.level = 0;
        rtc_set_alarm_aml(&pdev->dev, &alarm_data);
        printk("[RTC]suspend %d times, system will up %ds later!\n", ++suspend_time, alarm_data.alarm_sec);
    }
    return 0;
}
...
static ssize_t show_alarm(struct class *class, struct class_attribute *attr, char *buf)
{
    int ret = 0;
    unsigned long cur_secs;
    struct rtc_time cur_time;
...
{
    unsigned long secs, cur_secs;
    struct rtc_time cur_time;
    unsigned data = 0;

    secs = simple_strtoul(buf, NULL, 0);
    aml_rtc_read_time(NULL, &cur_time);
    rtc_tm_to_time(&cur_time, &cur_secs);
    printk("[RTC] set_alarm(%ld)\n", secs);
    if(secs > 0) {
        s_alarm_secs = cur_secs + secs + 3;
    } else {
        s_alarm_secs = 0;
    }
...
static struct class_attribute rtc_class_attrs[] = {
    ...
    __ATTR(alarm, S_IRUGO | S_IWUSR, show_alarm, store_alarm),
    __ATTR_NULL
};
