...
static int g_powerkey_led = 0;

static void set_power_led_onoff(int onoff)// 1:on; 0:off
{
	/* GPIO AO_10 */
    if (1 == onoff) { //led on
		printk("GPIO: set power led on\n");
...
static ssize_t gpio_powerkey_led_store(struct class *cla, struct class_attribute *attr, const char *buf, size_t count)
{
	...
	onoff = simple_strtoul(buf, NULL, 0);
	if (onoff >= 0 && onoff <= 1) {
		printk("powerkey_led: %d -> %d\n", g_powerkey_led, onoff);
		g_powerkey_led = onoff;
		set_power_led_onoff(g_powerkey_led);
	} else {
		printk("set powerkey_led error\n");
	}
...
static inline int _gpio_setup_bank_bit(cmd_t  *op)
{
    ...
    case 'o': //bank AO
        op->bank = GPIOAO_bank_bit0_11(0);
        if (op->bit < 12) { //bit0..11
