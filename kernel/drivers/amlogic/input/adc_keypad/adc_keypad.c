...
#if 1
#define KEYPAD_LED_CONTROL_TIME 200
    kp_work(kp_data);
    ...
    if (kp_data->led_control ){
		...
		if (KEYPAD_LED_CONTROL_TIME == timer_count){
			kp_data->led_control_param[0] = 0;
			timer_count = 0;
			kp_data->led_control(kp_data->led_control_param);
		}
    }
#else
    unsigned int result;
...
#endif
	kp->led_control_param[0] = 0;
    kp->led_control(kp->led_control_param);
