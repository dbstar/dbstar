...
public class ImageSet {
		...
		if (appUri != null && !appUri.equals("")) {			
			...
			if (appFile.exists()) {			
				try {
					...
					bitmapOptions.inSampleSize = (int) ((bitmapOptions.outWidth) * 1.0 / 345);
					bitmapOptions.inJustDecodeBounds = false;
					
					bitmap = BitmapFactory.decodeFile(appUri, bitmapOptions);
				} catch (OutOfMemoryError e) {
					Log.d("ImageSet", " appBitmap out of memory error = " + e);
					e.printStackTrace();
				}
			}
		}
	...
	static class ImageWork extends AsyncTask<String, Void, Bitmap> {
